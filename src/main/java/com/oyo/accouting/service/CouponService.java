...
public class CouponService {
    ...
    public String expire() {
        ...
        if(CollectionUtils.isNotEmpty(couponMetadataList)){
            ...
            for(CouponMetadata  couponMetadata : couponMetadataList){
                doExpire(couponMetadata);
            }
        }
        return null;
    }
    ...
    public void doExpire(CouponMetadata  couponMetadata){
        List<Coupons> coupons;
        do{
            PageHelper.startPage(1, PAGE_SIZE);
            coupons = couponsMapper.selectByConditions(MapUtils.createConditionMap("metadataIdList",Arrays.asList(couponMetadata.getId()),"statusList", Arrays.asList(CouponStatus.UNUSED.getStatus())));
            expireConpon(coupons);
        }while (coupons != null &&  coupons.size() == PAGE_SIZE);

        couponMetadataMapper.updateByConditions(MapUtils.createConditionMap("idList",Arrays.asList(couponMetadata.getId()),"statusList",Arrays.asList(CouponMetadataStatus.READY.getStatus(),CouponMetadataStatus.ONLINE.getStatus()),"targetStatus",CouponMetadataStatus.OFFLINE.getStatus(),"lastUpdateTime",DateUtils.getCurrentDate()));
    }

    public void expireConpon(List<Coupons> coupons){
        if(CollectionUtils.isNotEmpty(coupons)){
            couponsMapper.updateByConditions(MapUtils.createConditionMap("idList", Lists.transform(coupons, new Function<Coupons,Long>() {
                @Override
                public Long apply(@Nullable Coupons o) {
                    return o.getId();
                }
